...
<input type="hidden" name="SALE_NAME" id="SALE_NAME" class="form-control" value="<?php echo $_POST["SALE_NAME"]; ?>">
<input type="hidden" name="SKU_CAT" id="SKU_CAT" class="form-control" value="<?php echo $_POST["SKU_CAT"]; ?>">
<div class="card">
...
<script>
    function DisplayGraph_Monthly() {
        ...
        let SKU_CAT = $("#SKU_CAT").val();

        $.post("engine/get_data_by_brand_list.php", {year: year , SKU_CAT: SKU_CAT , SALE_NAME: SALE_NAME}, function (data) {
            console.log(data);
            let BRAND = [];
            let loop = 1;
            for (let i in data) {
                BRAND.push(data[i].BRAND);
                showGraph_Monthly(loop, data[i].BRAND);
                loop++;
            }
        });
    }
...
<script>
    function showGraph_Monthly(graph_number, BRAND) {
