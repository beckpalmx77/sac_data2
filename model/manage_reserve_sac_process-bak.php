...
## Search
    ...
    $searchQuery .= " AND DI_DATE BETWEEN '" . date("Y/m/d") . "' AND '" . date("Y/m/d") . "'";

## Total number of records without filtering
    $stmt = $conn_sqlsvr->prepare($sql_reserve);
    $stmt->execute();
    $records = $stmt->fetch();
    $totalRecords = $records['allcount'];

## Total number of records with filtering
    $stmt = $conn_sqlsvr->prepare($sql_reserve . $searchQuery);
    $stmt->execute($searchArray);
    $records = $stmt->fetch();
    $totalRecordwithFilter = $records['allcount'];

## Fetch records
    $sql_get_data = $sql_reserve . $searchQuery
        . " ORDER BY DI_DATE asc "
        . " OFFSET " . (int)$row . " ROWS FETCH NEXT " . (int)$rowperpage . " ROWS ONLY";

    //. " | offset = " . (int)$row . " | limit = " . (int)$rowperpage;
    //$myfile = fopen("getdata-param.txt", "w") or die("Unable to open file!");
    //fwrite($myfile,  $sql_get_data);
    //fclose($myfile);

    $stmt = $conn_sqlsvr->prepare($sql_get_data);
