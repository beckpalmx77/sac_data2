...
## Fetch records
    $sql_get = "SELECT v_wh_stock_balance.*,wh_product_master.product_name FROM v_wh_stock_balance
        LEFT JOIN wh_product_master ON wh_product_master.product_id = v_wh_stock_balance.product_id
        WHERE 1 ";

    $sql_order = " ORDER BY product_id " . " LIMIT :limit,:offset";

    $stmt = $conn->prepare($sql_get . $searchQuery . $sql_order);

/*
    $txt = "sql = " . $sql_get . $searchQuery . $sql_order;
